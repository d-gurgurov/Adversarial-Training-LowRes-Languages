...
# loading datasets
def load_and_prepare_data(languages):
...
def train_model(model, optimizer, scheduler, criterion, train_loader, val_loader, num_epochs=10, model_save_path='best_multi_lang_model.pth'):
...
model_save_path='best_multi_lang_model.pth'
...
results_file = 'results/multi_sent_results.txt'
os.makedirs(os.path.dirname(results_file), exist_ok=True)
with open(results_file, 'a') as f:
    f.write(f'Combined Test Set Results - Seed: {args.seed}, Num Epochs: {args.num_epochs}\n')
    f.write(f'Test Loss: {test_loss:.4f}, Test Sentiment F1: {test_f1:.4f}\n\n')

for i, language in enumerate(args.languages):
    language_test_dataset = Dataset.from_pandas(load_dataset(f"DGurgurov/{language}_sa")['test'].to_pandas())
    
    tokenized_language_test_dataset = language_test_dataset.map(tokenize_function, batched=True)
    tokenized_language_test_dataset.set_format(type='torch', columns=['input_ids', 'attention_mask', 'label'])
    
    language_test_loader = create_data_loader(tokenized_language_test_dataset, batch_size=64)
    
    lang_test_loss, lang_test_f1 = evaluate_model(model, language_test_loader)
    
    print(f'Test Results for {language}: '
          f'Test Loss: {lang_test_loss:.4f}, '
          f'Test Sentiment F1: {lang_test_f1:.4f}')
    
    with open(results_file, 'a') as f:
        f.write(f'Language: {language} - Seed: {args.seed}, Num Epochs: {args.num_epochs}\n')
        f.write(f'Test Loss: {lang_test_loss:.4f}, Test Sentiment F1: {lang_test_f1:.4f}\n\n')
